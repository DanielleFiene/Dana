...
`npm run backtest` replays the current score on labelled DANA days (Mallorca 28 Oct 2024, Magre 2024, Magre aftermath 30 Oct–2 Nov, Catalonia 4 Nov 2024, Almería 11 Nov 2024, Málaga 13 Nov 2024, Murcia Sep 2023, plus dry controls) against Open-Meteo historical forecasts. That is a **near-analysis** stitch, not a 48-hour forecast. Model rain at T−24/T−48/T−72 is printed separately. SAIH gauges are not in this loop yet. Unlabelled Magre squares (Alacant, Vega Baja, Gibraltar, …) stay out of hit/miss counts until a documented flood exists for that day.

Misses and false alarms are tagged with a **desk mechanism** (`grid-undercatch` vs `hangover` vs `leftover-rain` vs `lead-time-dry`) so a later threshold change cannot “fix” a convective-core miss with the hangover hinge, or vice versa. Burn scars and infiltration belong to catchment hydrology, not to those knobs. Labelled days also log **margin to the nearest watch-3 path** (setup+impact 0.48/0.38, severe impact 0.64, corridor upscale 22 mm/24 h or 45 mm/48 h) so a thin hangover or a comfortable undercatch-hit is visible before the next binary fail. `thin` is the binding path within 12 %, not a side limb.

The map shows a **situation**, not a chance figure and not an instruction. Open-Meteo’s precipitation probability is an input to the rain bundle only. A percentage on the desk waits for ECMWF IFS ENS (51 members at 0.25°) **calibrated with SAIH later**. SAIH does not admit AROME into the live score.

**AROME** is a parallel backtest source. Live scoring stays on the ICON/ECMWF mix. Magre: less wrong on Turís/Chiva/Utiel (best cell ~240 mm vs ~700–770 mm observed — still ~3× short) and worse on l’Horta. That is a bias profile, not a fix. `corridorBelt` is a table label only. Do not say “rule” before a majority across `INLAND_AROME_RULE_MIN_CELLS` (6) **inland-orographic** labelled cells. Mallorca 28 Oct (island) and Almería 11 Nov (south, outside AROME) are labelled; they do not increment that count (still 1 of 6 inland-orographic). Murcia Sep 2023 AROME all-null is an **Open-Meteo archive gap**, not an AROME dry-miss and not the inland/coast count. The 31 Oct Utiel AROME false alarm is leftover-rain with `source: arome`, not a per-model FA bucket.
